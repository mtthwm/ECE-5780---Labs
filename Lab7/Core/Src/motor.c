...
void PI_update(void) {
     ...
     */
		...
		#define ENCODER_PERIOD 0.045 / 60
    
    /// TODO: calculate error signal and write to "error" variable
		// Motor Speed in RPM = motor_speed / 64;
		
		// error = (motor_speed / (64 * ENCODER_PERIOD)) - target_rpm; // Error in RPM
		error = target_rpm - (motor_speed/2); // Error in Encoder Ticks
